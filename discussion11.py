...
def add_pets_from_json(filename, cur, conn):
    ...
    for row in json_data:
        cur.execute("INSERT INTO Patients (id,name,species_id,age,cuteness,aggressiveness) VALUES (?,?,?,?,?,?)",(count,row["name"], row["species"], row["age"], row["cuteness"], row["aggressiveness"]))
        count += 1
    conn.commit()
...
def non_aggressive_pets(aggressiveness, cur, conn):
    tame_pets = cur.execute("SELECT name FROM Patients WHERE aggressiveness < " + str(aggressiveness))
    return tame_pets
